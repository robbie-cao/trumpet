...
{	
	...
	// Initiate NuVox63Ex audio decode lib with callback functions stored in g_asAppCallBack[0]
	NuVox63ExApp_DecodeInitiate(&g_sApp.sNuVox63ExApp, (UINT8 *)&g_sApp.uTempBuf, 0);
...
{
	// Start NuVox63Ex decode lib to decode NuVox63Ex file stored from current audio id and played from audio channel 0.
	// The ROM file is placed on SPI Flash address "AUDIOROM_STORAGE_START_ADDR".
	// And decode the first frame of PCMs.
	if ( NuVox63ExApp_DecodeStartPlayByID(&g_sApp.sNuVox63ExApp, g_sApp.u32PlayID, AUDIOROM_STORAGE_START_ADDR, 0) == FALSE )
		return FALSE;
...
BOOL App_StopPlay(void)
{
	...
	NuVox63ExApp_DecodeStopPlay( &g_sApp.sNuVox63ExApp);
...
BOOL App_ProcessPlay(void)
{
	...
	// Continue decode NuVox63Ex data to produce PCMs for audio playback.
	if ( NuVox63ExApp_DecodeProcess(&g_sApp.sNuVox63ExApp) == TRUE )
