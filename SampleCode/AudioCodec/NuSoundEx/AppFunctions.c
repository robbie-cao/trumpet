...
{	
	...
	// Initiate NuSoundEx audio decode lib with callback functions stored in g_asAppCallBack[0].
	NuSoundExApp_DecodeInitiate(&g_sApp.sNuSoundExApp, (UINT8 *)&g_sApp.uTempBuf, 0);
...
{
	// Start NuSoundEx decode lib to decode NuSoundEx file stored from current audio id and played from audio channel 0.
	// The ROM file is placed on SPI Flash address "AUDIOROM_STORAGE_START_ADDR".
	// And decode the first frame of PCMs.
	if ( NuSoundExApp_DecodeStartPlayByID(&g_sApp.sNuSoundExApp, g_sApp.u32PlayID, AUDIOROM_STORAGE_START_ADDR, 0) == FALSE )
		return FALSE;
...
BOOL App_StopPlay(void)
{
	...
	NuSoundExApp_DecodeStopPlay( &g_sApp.sNuSoundExApp);
...
BOOL App_ProcessPlay(void)
{
	...
	// Continue decode NuSoundEx data to produce PCMs for audio playback.
	if ( NuSoundExApp_DecodeProcess(&g_sApp.sNuSoundExApp) == TRUE )
