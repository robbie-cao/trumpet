...
void App_Initiate(void)
{
	...
	// Initiate LP8 audio decode lib with callback functions stored in g_asAppCallBack[0]
	LP8App_DecodeInitiate(&g_sApp.sLP8App, NULL, 0);
...
{
	// Start LP8 decode lib to decode LP8 file stored from current audio id and played from audio channel 0.
	// The ROM file is placed on SPI Flash address "AUDIOROM_STORAGE_START_ADDR".
	// And decode the first frame of PCMs.
	if ( LP8App_DecodeStartPlayByID(&g_sApp.sLP8App, g_sApp.u32PlayID, AUDIOROM_STORAGE_START_ADDR, 0) == FALSE )
		return FALSE;
...
BOOL App_StopPlay(void)
{
	...
	LP8App_DecodeStopPlay( &g_sApp.sLP8App);
...
BOOL App_ProcessPlay(void)
{
	...
	// Continue decode LP8 data to produce PCMs for audio playback.
	if ( LP8App_DecodeProcess(&g_sApp.sLP8App) == TRUE )
