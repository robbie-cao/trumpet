...
{	
	...
	// Initiate NuVox53Ex audio decode lib with callback functions stored in g_asAppCallBack[0]
	NuVox53ExApp_DecodeInitiate(&g_sApp.sNuVox53ExApp, (UINT8 *)&g_sApp.uTempBuf, 0);
...
{
	// Start NuVox53Ex decode lib to decode NuVox53Ex file stored from current audio id and played from audio channel 0.
	// The ROM file is placed on SPI Flash address "AUDIOROM_STORAGE_START_ADDR".
	// And decode the first frame of PCMs.
	if ( NuVox53ExApp_DecodeStartPlayByID(&g_sApp.sNuVox53ExApp, g_sApp.u32PlayID, AUDIOROM_STORAGE_START_ADDR, 0) == FALSE )
		return FALSE;
...
BOOL App_StopPlay(void)
{
	...
	NuVox53ExApp_DecodeStopPlay( &g_sApp.sNuVox53ExApp);
...
BOOL App_ProcessPlay(void)
{
	...
	// Continue decode NuVox53Ex data to produce PCMs for audio playback.
	if ( NuVox53ExApp_DecodeProcess(&g_sApp.sNuVox53ExApp) == TRUE )
