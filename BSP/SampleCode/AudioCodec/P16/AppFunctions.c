...
void App_Initiate(void)
{
	...
	// Initiate P16 audio decode lib with callback functions stored in g_asAppCallBack[0]
	P16App_DecodeInitiate(&g_sApp.sP16App, NULL, 0);
...
{
	// Start P16 decode lib to decode P16 file stored from current audio id and played from audio channel 0.
	// The ROM file is placed on SPI Flash address "AUDIOROM_STORAGE_START_ADDR".
	// And decode the first frame of PCMs.
	if ( P16App_DecodeStartPlayByID(&g_sApp.sP16App, g_sApp.u32PlayID, AUDIOROM_STORAGE_START_ADDR, 0) == FALSE )
		return FALSE;
...
BOOL App_StopPlay(void)
{
	...
	P16App_DecodeStopPlay( &g_sApp.sP16App);
...
BOOL App_ProcessPlay(void)
{
	...
	// Continue decode P16 data to produce PCMs for audio playback.
	if ( P16App_DecodeProcess(&g_sApp.sP16App) == TRUE )
